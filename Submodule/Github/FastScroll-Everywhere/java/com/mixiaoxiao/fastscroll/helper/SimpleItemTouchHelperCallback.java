...
public class SimpleItemTouchHelperCallback extends ItemTouchHelper.Callback {
    ...
    public interface Validator {
        ...
        boolean isSwipable(int position);
    }
    ...
    private boolean isMovable(int position) {
        if(mValidator!=null) {
            return mValidator.isMovable(position);
        } else {
            return false;
        }
    }
    ...
    private boolean isSwipable(int position) {
        if(mValidator!=null) {
            return mValidator.isSwipable(position);
        } else {
            return false;
        }
    }
    ...
    public int getMovementFlags(RecyclerView recyclerView, RecyclerView.@NotNull ViewHolder viewHolder) {
        ...
        if (recyclerView.getLayoutManager() instanceof GridLayoutManager) {
            final int dragFlags = isMovable(viewHolder.getAdapterPosition())?ItemTouchHelper.UP | ItemTouchHelper.DOWN | ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT:0;
            final int swipeFlags = 0;
            return makeMovementFlags(dragFlags, swipeFlags);
        } else {
            final int dragFlags = isMovable(viewHolder.getAdapterPosition())?ItemTouchHelper.UP | ItemTouchHelper.DOWN:0;
            final int swipeFlags = isSwipable(viewHolder.getAdapterPosition())?ItemTouchHelper.START | ItemTouchHelper.END:0;
            return makeMovementFlags(dragFlags, swipeFlags);
        }
    }
